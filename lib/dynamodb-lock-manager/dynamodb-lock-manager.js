...
var DynamoDBLockManager = /** @class */ (function () {
    function DynamoDBLockManager(aDbClient, aLockTableName) {
        this.myLockUUID = '';
        this.lockSecsToLive = 10; // a lock is only held for N seconds
        this.lockRefreshSecs = this.lockSecsToLive / 2; // we will attempt to extend each active lock at the mid-life of the lock
        ...
        this.deleteMyLockDelaySecs = 1;
        this.lockTable = '';
        this.milliSecsPerSecs = 1000;
        this.blockingLocks = new Map();
        this.myActiveLocks = new Map();
        this.myActiveLockTimers = new Map();
        if (!aDbClient) {
    ...
    DynamoDBLockManager.prototype.putItem = function (params, callback) {
        try {
            ...
            this.dbClient.put(params, function (err, data) {
                if (err) {
    ...
    DynamoDBLockManager.prototype.updateItem = function (params, callback) {
        try {
            ...
            this.dbClient.update(params, function (err, data) {
                if (err) {
                    callback(err, null);
                }
                else {
